    ...
    public String getEscaleras() {
        ...
        for (Pinta pinta : Pinta.values()){
            ...
            for (int i = 0; i < valores.length - 4; i++) {
                if (valores[i] != 0 && valores[i + 1] != 0 && valores[i + 2] != 0 && valores[i + 3] != 0 && valores[i + 4] != 0) {
                    if (!hayEscaleras) {
                        hayEscaleras = true;
                        mensaje = new StringBuilder("Las escaleras encontradas son:\n");
                    }
                    mensaje.append("Escalera de ").append(NombreCarta.values()[i]).append(" a ").append(NombreCarta.values()[i + 4]).append(" de ").append(pinta).append("\n");
                }
            }
        }
